...
void keymap_handler(int idt_index) {
    ...
    // 终端8259a在初始化的时候已经让自动 end of interrupt 了，详情见kernel.asm
    // 经过测试是不需要手动进行eoi的，除非初始化时（kernel.asm中）设定非eoi模式
    //send_eoi(idt_index);

    // 0x60端口是在IBM PC体系结构中规定的。在该体系结构中，键盘控制器芯片负责处理键盘和鼠标输入，并将数据发送到固定的I/O端口地址。
    // 这种设计在许多现代计算机上仍然存在，并且仍然使用相同的端口地址0x60来接收键盘输入。由于PC体系结构已成为业界标准，
    // 因此该端口在许多其他计算机体系结构中也得到了采用。
    uchar scancode = in_byte(0x60);

    // 是扩展码字节
    // 当用户按下右侧的Shift、Alt或Ctrl键时，键盘控制器会发送0xe0作为前缀，然后发送另一个扫描码，以表示相应的键。
    if (scancode == 0xe0)
    {
        // 置扩展状态
        extcode_state = true;
    ...
    if (extcode_state)
    {
        ...
        // 扩展状态无效，已经让ext=3了，表示这个扩展状态已经处理了
        extcode_state = false;
    }
